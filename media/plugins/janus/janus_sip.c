...
void janus_sip_create_client(janus_plugin_session *handle, int *error);
...
void janus_sip_destroy_client(janus_plugin_session *handle, int *error);
json_t *janus_sip_query_client(janus_plugin_session *handle);
...
static janus_plugin janus_sip_plugin =
	JANUS_PLUGIN_INIT (
		...
		.create_session = janus_sip_create_client,
		...
		.destroy_session = janus_sip_destroy_client,
		.query_session = janus_sip_query_client,
	);
...
#define JANUS_SIP_TRANSFER_MODE_BLIND     1
#define JANUS_SIP_TRANSFER_MODE_ATTENDED  2
...
static struct janus_json_parameter transfer_parameters[] = {
    {"mode", JSON_INTEGER, 0},
    /* Line that needs to be transfered */
    {"line", JSON_INTEGER, 0},
    /* blind transfer parameters */
	{"target", JSON_STRING, 0},
    /* attended transfer */
    {"target_line", JSON_INTEGER, 0}
};
static struct janus_json_parameter hold_parameters[] = {
    {"line", JSON_INTEGER, 0}
};
static struct janus_json_parameter hangup_parameters[] = {
    {"line", JSON_INTEGER, 0}
};
...
/* @pitzlabs transfer states */
typedef enum {
    janus_sip_transfer_idle = 0,
    janus_sip_transfer_initiated,
    janus_sip_transfer_transferee_accepted,
    janus_sip_transfer_inprogress,
    janus_sip_transfer_complete

} janus_sip_transfer_status;
...
typedef struct janus_sip_session {
    /* @pitzlabs multi sessions */
	/* ssip_t *stack; */
	void *account_handle; 
    int sess_id;
	janus_sip_call_status status;
    /* @pitzlabs transfer state */
    janus_sip_transfer_status transfer_status;
	janus_sip_media media;
	char *callee;
    /* sip dialog parameters */
	char *callid;
    char *from_tag;
    char *to_tag;
...
#define MAX_SIP_SESSIONS_PER_CLIENT   4
...
typedef struct janus_sip_account {
    ...
    /* @pitzlabs multiple sessions */
	ssip_t *stack;
    nua_handle_t *s_nh_r;
    janus_sip_session sessions[MAX_SIP_SESSIONS_PER_CLIENT];
    janus_mutex session_lock;
} janus_sip_account;

static GHashTable *clients;  /* SIP User agents */
static GList *old_clients;
...
void janus_sip_init_session(janus_sip_account *account, janus_sip_session *session, int index);
void janus_sip_hangup_session(janus_sip_account *account, janus_sip_session *session);
void janus_sip_handle_rtp(janus_sip_account *account, janus_sip_session *session, int video, char *buf, int len);
void janus_sip_handle_rtcp(janus_sip_account *account, janus_sip_session *session, int video, char *buf, int len);
void janus_sip_lazy_clean_session(janus_sip_session *session);
janus_sip_session *janus_sip_get_idle_session(janus_sip_account *account);
janus_sip_session *janus_sip_find_session_from_handle(janus_sip_account *account, nua_handle_t *handle);
janus_sip_session *janus_sip_find_session(janus_sip_account *account, int id);
void janus_sip_debug_client(janus_sip_account *account);
...
static void *janus_sip_watchdog(void *data) {
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		if(old_clients != NULL) {
			GList *sl = old_clients;
			JANUS_LOG(LOG_HUGE, "Checking %d old SIP sessions...\n", g_list_length(old_clients));
			while(sl) {
				janus_sip_account *account = (janus_sip_account *)sl->data;
				if(!account) {
					sl = sl->next;
					continue;
				}
                if (now-account->destroyed >= 5*G_USEC_PER_SEC) {
                    JANUS_LOG(LOG_VERB, "Freeing old SIP client\n");
                    GList *rm = sl->next;
                    old_clients = g_list_delete_link(old_clients, sl);
                    sl = rm;
                    
                    if (account->identity) {
					    g_free(account->identity);
                        account->identity = NULL;
					}
                    account->force_udp = TRUE;
                    account->sips = TRUE;
					if (account->proxy) {
					    g_free(account->proxy);
					    account->proxy = NULL;
					}
					if (account->outbound_proxy) {
					    g_free(account->outbound_proxy);
					    account->outbound_proxy = NULL;
					}
					if (account->secret) {
					    g_free(account->secret);
					    account->secret = NULL;
					}
					if (account->username) {
					...
					if (account->authuser) {
					    g_free(account->authuser);
					    account->authuser = NULL;
					}
                    if (account->transaction) {
                        g_free(account->transaction);
                        account->transaction = NULL;
                    }
                  
                    int i = 0;
                    for(i=0; i < MAX_SIP_SESSIONS_PER_CLIENT; i++) {
                        janus_sip_lazy_clean_session(&account->sessions[i]);
                    }                
                    account->handle = NULL;
					g_free(account);
					account = NULL;
...
void janus_sip_lazy_clean_session(janus_sip_session *session) {
    ...
    if (session->from_tag) {
        g_free(session->from_tag);
        session->from_tag = NULL;
    }
    if (session->to_tag) {
        g_free(session->to_tag);
        session->to_tag = NULL;
    }
    if (session->sdp) {
...
static void janus_sip_sofia_logger(void *handle, char const *fmt, va_list ap) {
	if(!fmt)
		return;

    janus_sip_account *account = (janus_sip_account *)handle;
    if(account == NULL) {
	...
	if(append) {
		...
		if(line[3] == '-') {
			if(!started) {
				...
				gateway->notify_event(&janus_sip_plugin, account->handle, info);
...
static janus_sip_account *janus_sip_lookup_client(janus_plugin_session *handle) {
    janus_sip_account *account = NULL;
    if (g_hash_table_contains(clients, handle)) {
        account = (janus_sip_account *)handle->plugin_handle;
    }
    return account;
}

/* @pitzlabs initializing clients */
void janus_sip_create_client(janus_plugin_session *handle, int *error) {
    if(g_atomic_int_get(&stopping) || !g_atomic_int_get(&initialized)) {
        *error = -1;
        return;
    }
    janus_sip_account *account = g_malloc0(sizeof(janus_sip_account));
    account->handle = handle;
    account->transaction = NULL;
    account->identity = NULL; 
    account->force_udp = FALSE;
    account->sips = TRUE;
    account->username = NULL;
    account->display_name = NULL;
    account->user_agent = NULL;
    account->authuser = NULL;
    account->secret = NULL;
    account->secret_type = janus_sip_secret_type_unknown;
    account->sip_port = 0;
    account->proxy = NULL;
    account->outbound_proxy = NULL;
    account->registration_status = janus_sip_registration_status_unregistered;
    account->stack = NULL;
    janus_mutex_init(&account->session_lock);
	account->destroyed = 0;

    int i = 0;
    for(i=0; i < MAX_SIP_SESSIONS_PER_CLIENT; i++) {
        janus_sip_init_session(account, &account->sessions[i], i);
    }

    handle->plugin_handle = account;
    janus_mutex_lock(&clients_mutex);
    g_hash_table_insert(clients, handle, account);
    janus_mutex_unlock(&clients_mutex);
...
void janus_sip_debug_client(janus_sip_account *account) {
    printf("Debuging Client ---- \n");
    int i =0;
    for(i=0; i < MAX_SIP_SESSIONS_PER_CLIENT; i++) {
        janus_sip_session *session = &account->sessions[i];
        printf("Session status(%d) id(%d)\n", session->status, session->sess_id);     
    }
    printf("---- Debug End \n");
}
...
void janus_sip_init_session(janus_sip_account *account, janus_sip_session *session, int index) {
	...
	account->registration_status = janus_sip_registration_status_unregistered;
    */
    session->account_handle = account;
	session->status = janus_sip_call_status_idle;
    /* @pitzlabs transfer status to idle */
	session->transfer_status = janus_sip_transfer_idle;
	session->sess_id = index;
	session->callee = NULL;
	session->callid = NULL;
	session->from_tag = NULL;
	session->to_tag = NULL;
...
void janus_sip_destroy_client(janus_plugin_session *handle, int *error) {
    ...
    janus_mutex_lock(&clients_mutex); 
    janus_sip_account *account = janus_sip_lookup_client(handle);;
    if(!account) {
        janus_mutex_unlock(&clients_mutex);
        JANUS_LOG(LOG_ERR, "No SIP account associated with this handle...\n");
        *error = -2;
        return;
    }
    ...
    if(!account->destroyed) {
        ...
        account->destroyed = janus_get_monotonic_time();
        g_hash_table_remove(clients, handle);
        if(account->stack != NULL) {
			/* Shutdown the NUA: this will remove the session later on */
			nua_shutdown(account->stack->s_nua);
		} else {
			/* No stack, maybe never registered: cleaning up and removing the session is done in a lazy way */
			old_clients = g_list_append(old_clients, account);
		}
        JANUS_LOG(LOG_VERB, "Destroying SIP client (%s)...\n", account->identity );
    }
    janus_mutex_unlock(&clients_mutex); 
    return;
}
...
janus_sip_session *janus_sip_get_idle_session(janus_sip_account *account) {
    ...
    janus_sip_session *idle_session = NULL;
    janus_sip_session *tmp_session = NULL;
    int i = 0;

    for(i=0; i < MAX_SIP_SESSIONS_PER_CLIENT; i++) {
        tmp_session = &account->sessions[i];        
        if(tmp_session->status == janus_sip_call_status_idle) {
            idle_session = tmp_session;
            break;
        } 
    }        
...
janus_sip_session *janus_sip_find_session_from_handle(janus_sip_account *account, nua_handle_t *handle) {
    ...
    for(i=0; i < MAX_SIP_SESSIONS_PER_CLIENT; i++) {
        session = &account->sessions[i];
        if(session->s_nh_i == handle) {
            return session;
        }
    }
...
json_t *janus_sip_query_client(janus_plugin_session *handle) {
	...
	json_t *info = json_object();
	json_object_set_new(info, "username", account->username ? json_string(account->username) : NULL);
	json_object_set_new(info, "authuser", account->authuser ? json_string(account->authuser) : NULL);
	json_object_set_new(info, "secret", account->secret ? json_string("(hidden)") : NULL);
	json_object_set_new(info, "display_name", account->display_name ? json_string(account->display_name) : NULL);
	json_object_set_new(info, "user_agent", account->user_agent ? json_string(account->user_agent) : NULL);
	json_object_set_new(info, "identity", account->identity ? json_string(account->identity) : NULL);
	json_object_set_new(info, "registration_status", json_string(janus_sip_registration_status_string(account->registration_status)));
    /*
...
void janus_sip_setup_media(janus_plugin_session *handle) {
	...
	if(!account) {
        janus_mutex_unlock(&clients_mutex);
		JANUS_LOG(LOG_ERR, "No account associated with this handle...\n");
		return;
	}

    /* TBD: handle setup media inside session */
	/* g_atomic_int_set(&session->hangingup, 0); */
...
void janus_sip_incoming_rtp(janus_plugin_session *handle, int video, char *buf, int len) {
    ...
    janus_sip_account *account = (janus_sip_account *)handle->plugin_handle;
 
    if(gateway) {
        for(i=0; i < MAX_SIP_SESSIONS_PER_CLIENT; i++) {
            janus_sip_handle_rtp(account, &account->sessions[i], video, buf, len);
        }
    } 
}
void janus_sip_incoming_rtcp(janus_plugin_session *handle, int video, char *buf, int len) {
    ...
    if(gateway) {
        for(i=0; i < MAX_SIP_SESSIONS_PER_CLIENT; i++) {
            janus_sip_handle_rtcp(account, &account->sessions[i], video, buf, len);
        }
    }
}
...
/* 
 * @pitzlabs
 * Incoming RTP: Send to all the active sip sessions. 
 */
void janus_sip_handle_rtp(janus_sip_account *account, janus_sip_session *session, int video, char *buf, int len) {

		/* Honour the audio/video active flags */
		if(!account || !session || account->destroyed) {
			JANUS_LOG(LOG_ERR, "No session associated with this handle...\n");
			return;
		}
				...
				if(session->media.has_srtp_local) {
					...
					if(res != srtp_err_status_ok) {
						...
						JANUS_LOG(LOG_ERR, "[SIP-%s] Video SRTP protect error... %s (len=%d-->%d, ts=%"SCNu32", seq=%"SCNu16")...\n",
							account->username, janus_srtp_error_str(res), len, protected, timestamp, seq);
					} else {
						...
						if(send(session->media.video_rtp_fd, sbuf, protected, 0) < 0) {
							...
							JANUS_LOG(LOG_HUGE, "[SIP-%s] Error sending SRTP video packet... %s (len=%d, ts=%"SCNu32", seq=%"SCNu16")...\n",
								account->username, strerror(errno), protected, timestamp, seq);
						}
					}
				} else {
					/* Forward the frame to the peer */
					if(send(session->media.video_rtp_fd, buf, len, 0) < 0) {
						...
						JANUS_LOG(LOG_HUGE, "[SIP-%s] Error sending RTP video packet... %s (len=%d, ts=%"SCNu32", seq=%"SCNu16")...\n",
							account->username, strerror(errno), len, timestamp, seq);
					}
				}
			}
		} else {
				...
				if(session->media.has_srtp_local) {
					...
					if(res != srtp_err_status_ok) {
						JANUS_LOG(LOG_ERR, "[SIP-%s] Video SRTCP protect error... %s (len=%d-->%d)...\n",
							account->username, janus_srtp_error_str(res), len, protected);
					} else {
						...
						if(send(session->media.video_rtcp_fd, sbuf, protected, 0) < 0) {
							JANUS_LOG(LOG_HUGE, "[SIP-%s] Error sending SRTCP video packet... %s (len=%d)...\n",
								account->username, strerror(errno), protected);
						}
					}
				} else {
					/* Forward the message to the peer */
					if(send(session->media.video_rtcp_fd, buf, len, 0) < 0) {
						JANUS_LOG(LOG_HUGE, "[SIP-%s] Error sending RTCP video packet... %s (len=%d)...\n",
							account->username, strerror(errno), len);
					}
				}
			}
		} else {
...
void janus_sip_hangup_media(janus_plugin_session *handle) {
    ...
    int i =0; 
    for(i=0; i < MAX_SIP_SESSIONS_PER_CLIENT; i++) {
        janus_sip_hangup_session(account, &account->sessions[i]);
    }    
    janus_mutex_unlock(&clients_mutex);
...
static void *janus_sip_handler(void *data) {
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		janus_mutex_lock(&clients_mutex);
        account = janus_sip_lookup_client(msg->handle);
		if(!account) {
			janus_mutex_unlock(&clients_mutex);
			JANUS_LOG(LOG_ERR, "No session associated with this handle...\n");
			janus_sip_message_free(msg);
		...
		janus_mutex_unlock(&clients_mutex);
        janus_sip_debug_client(account);
        ...
        */
		...
		if(!strcasecmp(request_text, "register")) {
			...
			if(account->registration_status > janus_sip_registration_status_unregistered && !refresh) {
				JANUS_LOG(LOG_ERR, "Already registered (%s)\n", account->username);
				error_code = JANUS_SIP_ERROR_ALREADY_REGISTERED;
				g_snprintf(error_cause, 512, "Already registered (%s)", account->username);
				goto error;
			}
			/* Parse the request */
			...
			if(refresh) {
				...
				if(account->proxy != NULL)
					g_free(account->proxy);
				account->proxy = NULL;
				if(account->outbound_proxy != NULL)
					g_free(account->outbound_proxy);
				account->outbound_proxy = NULL;
				if(account->user_agent != NULL)
					g_free(account->user_agent);
				account->user_agent = NULL;
				account->registration_status = janus_sip_registration_status_unregistered;
			}
			account->identity = g_strdup(username_text);
            account->force_udp = force_udp;
			account->sips = sips;
			account->username = g_strdup(user_id);
			account->authuser = g_strdup(authuser_text ? authuser_text : user_id);
			account->secret = secret_text ? g_strdup(secret_text) : NULL;
			account->secret_type = secret_type;
			if(display_name_text) {
				account->display_name = g_strdup(display_name_text);
			}
			if(user_agent_text) {
				account->user_agent = g_strdup(user_agent_text);
			}
			if(proxy_text) {
				account->proxy = g_strdup(proxy_text);
			}
			if(obproxy_text) {
				account->outbound_proxy = g_strdup(obproxy_text);
			}

			account->registration_status = janus_sip_registration_status_registering;
			if(!refresh && account->stack == NULL) {
				/* Start the thread first */
				GError *error = NULL;
				char tname[16];
				g_snprintf(tname, sizeof(tname), "sip %s", account->username);
				g_thread_try_new(tname, janus_sip_sofia_thread, account, &error);
				if(error != NULL) {
				...
				long int timeout = 0;
				while(account->stack == NULL || account->stack->s_nua == NULL) {
					g_usleep(100000);
					timeout += 100000;
					if(timeout >= 2000000) {
						break;
			...
			if(account->s_nh_r != NULL) {
				nua_handle_destroy(account->s_nh_r);
				account->s_nh_r = NULL;
			}
			...
			if(send_register) {
				...
				account->s_nh_r = nua_handle(account->stack->s_nua, account, TAG_END());
				if(account->s_nh_r == NULL) {
				...
				nua_register(account->s_nh_r,
					NUTAG_M_USERNAME(account->authuser),
					NUTAG_M_DISPLAY(account->display_name),
				...
				account->registration_status = janus_sip_registration_status_disabled;
				result = json_object();
				json_object_set_new(result, "event", json_string("registered"));
				json_object_set_new(result, "username", json_string(account->username));
				json_object_set_new(result, "register_sent", json_false());
				/* Also notify event handlers */
				if(notify_events && gateway->events_is_enabled()) {
					json_t *info = json_object();
					json_object_set_new(info, "event", json_string("registered"));
					json_object_set_new(info, "identity", json_string(account->identity));
			...
			if(account->registration_status < janus_sip_registration_status_registered) {
			...
			/* Unregister now */
			account->registration_status = janus_sip_registration_status_unregistering;
			nua_unregister(account->s_nh_r, TAG_END());
			result = json_object();
            ...
            /* Get an idle session */
            session = janus_sip_get_idle_session(account);
            if(session==NULL) {
				JANUS_LOG(LOG_ERR, "Max Sessions reached \n");
                error_code = JANUS_SIP_ERROR_WRONG_STATE;
				g_snprintf(error_cause, 512, "Maximum Sessions reached");
                goto error;
            }
			...
			char from_hdr[1024];
			if (account->display_name) {
				g_snprintf(from_hdr, sizeof(from_hdr), "\"%s\" <%s>", account->display_name, account->identity);
			} else {
				g_snprintf(from_hdr, sizeof(from_hdr), "%s", account->identity);
			}
			/* Prepare the stack */
			if(session->s_nh_i != NULL)
				nua_handle_destroy(session->s_nh_i);
			session->s_nh_i = nua_handle(account->stack->s_nua, account, TAG_END());
			if(session->s_nh_i == NULL) {
			...
			if(authuser) {
				...
				g_free(account->authuser);
				account->authuser = g_strdup(json_string_value(authuser));
			}
			if(secret) {
				JANUS_LOG(LOG_VERB, "Updating credentials (secret) for authenticating the INVITE\n");
				g_free(account->secret);
				account->secret = g_strdup(json_string_value(secret));
				account->secret_type = janus_sip_secret_type_plaintext;
			} else if(ha1_secret) {
				JANUS_LOG(LOG_VERB, "Updating credentials (ha1_secret) for authenticating the INVITE\n");
				g_free(account->secret);
				account->secret = g_strdup(json_string_value(ha1_secret));
				account->secret_type = janus_sip_secret_type_hashed;
			}
			/* Send INVITE */
			session->callee = g_strdup(uri_text);
			session->callid = g_strdup(callid);
			session->media.autoack = do_autoack;
			nua_invite(session->s_nh_i,
				SIPTAG_FROM_STR(from_hdr),
				SIPTAG_TO_STR(uri_text),
				SIPTAG_CALL_ID_STR(callid),
				SOATAG_USER_SDP_STR(sdp),
				NUTAG_PROXY(account->outbound_proxy),
				TAG_IF(strlen(custom_headers) > 0, SIPTAG_HEADER_STR(custom_headers)),
				NUTAG_AUTOANSWER(0),
			...
			if(account->transaction)
				g_free(account->transaction);
			account->transaction = msg->transaction ? g_strdup(msg->transaction) : NULL;
			/* Send an ack back */
			result = json_object();
			json_object_set_new(result, "event", json_string("calling"));
            json_object_set_new(result, "line", json_integer(session->sess_id));
            ...
            json_t *line_str = json_object_get(root, "line");
            if(line_str) {
                line = json_integer_value(line_str);
            }
            session = janus_sip_find_session(account, line);
            if(session==NULL) {
                JANUS_LOG(LOG_ERR, "Session not found \n");
                error_code = JANUS_SIP_ERROR_WRONG_STATE;
                g_snprintf(error_cause, 512, "Session not found");
                goto error;
            }
			...
			if(answer) {
				...
				GError *error = NULL;
				char tname[16];
				g_snprintf(tname, sizeof(tname), "siprtp %s", account->username);
			...
			nua_respond(session->s_nh_i, response_code, sip_status_phrase(response_code), TAG_END());
			/* Also notify event handlers */
			if(notify_events && gateway->events_is_enabled()) {
				json_t *info = json_object();
            ...
            /* @pitzlabs
               hold the call only if call status is in-call */ 
			if(!(session->status == janus_sip_call_status_incall)) {
            ...
            JANUS_VALIDATE_JSON_OBJECT(root, hangup_parameters,
                error_code, error_cause, TRUE,
                JANUS_SIP_ERROR_MISSING_ELEMENT, JANUS_SIP_ERROR_INVALID_ELEMENT);
            if(error_code != 0)
                goto error;
			...
			session->media.on_hold = FALSE;
			session->status = janus_sip_call_status_closing;
			nua_bye(session->s_nh_i, TAG_END());
			g_free(session->callee);
			session->callee = NULL;
			/* Notify the operation */
			result = json_object();
			...
			if(!strcasecmp(action_text, "start")) {
				...
				if(record_peer_audio || record_peer_video) {
					JANUS_LOG(LOG_INFO, "Starting recording of peer's %s (user %s, call %s)\n",
						...
						account->username, account->transaction);
					...
					if(record_peer_audio) {
						...
						if(recording_base) {
							...
							g_snprintf(filename, 255, "sip-%s-%s-%"SCNi64"-peer-audio",
								...
								account->transaction ? account->transaction : "unknown",
								now);
							/* FIXME This only works if offer/answer happened */
							session->arc_peer = janus_recorder_create(NULL, session->media.audio_pt_name, filename);
							if(session->arc_peer == NULL) {
				...
				if(record_audio || record_video) {
					...
					if(record_video) {
						...
						gateway->relay_rtcp(account->handle, 1, buf, 12);
					}
				}
			} else {
        ...
        /* @pitzlabs transfer */
            ...
            int mode = JANUS_SIP_TRANSFER_MODE_BLIND; 
            json_t *mode_str = json_object_get(root, "mode");
            if(mode_str != NULL) {
                mode = json_integer_value(mode_str);
            }
            json_t *line_str = json_object_get(root, "line");
            if(line_str) {
                line = json_integer_value(line_str);
            ...
            if(mode == JANUS_SIP_TRANSFER_MODE_BLIND) {
                /* We need to perform transfer attended/un-attended */
                if(!(session->status == janus_sip_call_status_incall)) {
                    JANUS_LOG(LOG_ERR, "Wrong state (not in a call? status=%s)\n", janus_sip_call_status_string(session->status));
                    /* Ignore */
                    janus_sip_message_free(msg);
                ...
                /* TBD: hold the call */
                json_t *uri = json_object_get(root, "target");
                const char *uri_text = json_string_value(uri);
                janus_sip_uri_t refer_uri;
                if (janus_sip_parse_uri(&refer_uri, uri_text) < 0) {
                    JANUS_LOG(LOG_ERR, "Invalid user address %s\n", uri_text);
                    error_code = JANUS_SIP_ERROR_INVALID_ADDRESS;
                    g_snprintf(error_cause, 512, "Invalid user address %s\n", uri_text);
                    goto error;
                ...
                /* change the status to initiated */
                session->transfer_status = janus_sip_transfer_initiated;
                nua_refer(session->s_nh_i,
                          SIPTAG_REFER_TO_STR(uri_text),
                          TAG_END());
 
                
            } else if(mode == JANUS_SIP_TRANSFER_MODE_ATTENDED) {

                json_t *target_line_str = json_object_get(root, "target_line");
                if(target_line_str == NULL) {
                    JANUS_LOG(LOG_ERR, "Target Line parameter not present \n");
                    error_code = JANUS_SIP_ERROR_WRONG_STATE;
                    g_snprintf(error_cause, 512, "Target Line parameter not present");
                    goto error;

                }
                int target_line = json_integer_value(target_line_str);
                janus_sip_session *target_session = janus_sip_find_session(account, target_line);
                if(target_session==NULL) {
                    JANUS_LOG(LOG_ERR, "Target session not found \n");
                    error_code = JANUS_SIP_ERROR_WRONG_STATE;
                    g_snprintf(error_cause, 512, "Target session not found");
                    goto error;
                }

                printf("Before sending Refer for attended transfer line_status(%d) target_status(%d)\n", session->status, target_session->status);
                printf("Target session callid(%s) to_tag(%s) from_tag(%s) \n", target_session->callid, target_session->to_tag, target_session->from_tag);
                printf("Refer To %s \n", target_session->callee);
                
                /* RFC 3891: Replaces */
                char replaces_str[255];
                g_snprintf(replaces_str, 
                           255, 
                           "%s;to-tag=%s;from-tag=%s", 
                           target_session->callid, 
                           target_session->to_tag,
                           target_session->from_tag);
                /*
                sip_replaces_t *replaces_hdr = g_malloc(sizeof(sip_replaces_t));
                replaces_hdr->rp_call_id = g_strdup(target_session->callid);
                replaces_hdr->rp_to_tag = g_strdup(target_session->to_tag);
                replaces_hdr->rp_from_tag = g_strdup(target_session->from_tag);
                */

                session->transfer_status = janus_sip_transfer_initiated;
                nua_refer(session->s_nh_i,
                          SIPTAG_REFER_TO_STR(target_session->callee),
                          SIPTAG_REPLACES_STR(replaces_str),
                          TAG_END());

            } else {
                JANUS_LOG(LOG_ERR, "Not supported mode \n");
                goto error;
            }
                        
			/* Send an ack back */
            result = json_object();
            json_object_set_new(result, "event", json_string("transfering"));
...
void janus_sip_sofia_callback(nua_event_t event, int status, char const *phrase, nua_t *nua, nua_magic_t *magic, nua_handle_t *nh, nua_hmagic_t *hmagic, sip_t const *sip, tagi_t tags[])
{
	janus_sip_account *account = (janus_sip_account *)magic;
	ssip_t *ssip = account->stack;

    janus_sip_session *session = janus_sip_find_session_from_handle(account, nh);
	...
	/* Status or Error Indications */
		case nua_i_active:
			JANUS_LOG(LOG_VERB, "[%s][%s]: %d %s\n", account->username, nua_event_name(event), status, phrase ? phrase : "??");
			break;
		case nua_i_error:
			JANUS_LOG(LOG_VERB, "[%s][%s]: %d %s\n", account->username, nua_event_name(event), status, phrase ? phrase : "??");
			break;
		...
		case nua_i_state:
            ...
            if(session == NULL) {
			...
			} else if (callstate == nua_callstate_proceeding &&
				    (session->s_nh_i == nh || session->s_nh_i == NULL)) {
                ...
                json_object_set_new(calling, "line", json_integer(session->sess_id));
				json_object_set_new(call, "result", calling);
				int ret = gateway->push_event(account->handle, &janus_sip_plugin, account->transaction, call, NULL);
				JANUS_LOG(LOG_VERB, "  >> Pushing event: %d (%s)\n", ret, janus_get_api_error(ret));
				json_decref(call);
				/* Also notify event handlers */
				if(notify_events && gateway->events_is_enabled()) {
				...
				g_free(account->transaction);
				account->transaction = NULL;
				gateway->close_pc(account->handle);
			}
			break;
	...
	/* SIP requests */
		...
		case nua_i_invite: {
            ...
            if (session == NULL) {
			    ...
			    char *caller_text = url_as_string(account->stack->s_home, sip->sip_from->a_url);
			    json_object_set_new(result, "caller", json_string(caller_text));
			    su_free(account->stack->s_home, caller_text);
			    ...
			    int ret = gateway->push_event(account->handle, &janus_sip_plugin, NULL, missed, NULL);
			...
			if(!reinvite) {
				/* New incoming call */
				session->callee = g_strdup(url_as_string(account->stack->s_home, sip->sip_from->a_url));
				session->callid = sip && sip->sip_call_id ? g_strdup(sip->sip_call_id->i_id) : NULL;
				session->status = janus_sip_call_status_invited;

                session->from_tag = sip && sip->sip_from ? g_strdup(sip->sip_from->a_tag) : NULL;
                session->to_tag = sip && sip->sip_to ? g_strdup(sip->sip_to->a_tag) : NULL;
                printf("SIP dialog tags (%s) (%s) url(%s)\n", sip->sip_from->a_tag, sip->sip_to->a_tag, sip->sip_to->a_url->url_user);
				/* Clean up SRTP stuff from before first, in case it's still needed */
				janus_sip_srtp_cleanup(session);
			}
			/* Parse SDP */
			...
			json_object_set_new(call, "result", calling);
			int ret = gateway->push_event(account->handle, &janus_sip_plugin, account->transaction, call, jsep);
			JANUS_LOG(LOG_VERB, "  >> Pushing event to peer: %d (%s)\n", ret, janus_get_api_error(ret));
			json_decref(call);
			if(jsep)
				json_decref(jsep);
		...
		case nua_i_info: {
			...
			int ret = gateway->push_event(account->handle, &janus_sip_plugin, NULL, info, NULL);
			JANUS_LOG(LOG_VERB, "  >> Pushing event to peer: %d (%s)\n", ret, janus_get_api_error(ret));
			json_decref(info);
			/* Send a 200 back */
			nua_respond(nh, 200, sip_status_phrase(200), TAG_END());
		...
		case nua_i_message: {
			...
			int ret = gateway->push_event(account->handle, &janus_sip_plugin, account->transaction, message, NULL);
			JANUS_LOG(LOG_VERB, "  >> Pushing event to peer: %d (%s)\n", ret, janus_get_api_error(ret));
			json_decref(message);
			/* Send a 200 back */
			nua_respond(nh, 200, sip_status_phrase(200), TAG_END());
		...
		case nua_i_options:
			...
			break;
        /* @pitzlabs Transfer Notify */
		case nua_i_notify:
	             JANUS_LOG(LOG_VERB, "[%s][%s]: %d %s\n", account->username, nua_event_name(event), status, phrase ? phrase : "??");
                     if((session->transfer_status == janus_sip_transfer_initiated) || 
                         (session->transfer_status == janus_sip_transfer_inprogress)) {
                         /* check if content type is message/sipfrag */
                         ...
                         session->media.on_hold = FALSE;
                         session->status = janus_sip_call_status_closing;  
                         /* send BYE, if NOTify with 200 Ok */
                         nua_bye(session->s_nh_i, TAG_END());
                         g_free(session->callee);
                         session->callee = NULL;
                         ...
                         /* Notify the operation */
                         json_t *info = json_object();
                         json_object_set_new(info, "event", json_string("transfercompleted"));
                         gateway->notify_event(&janus_sip_plugin, account->handle, info);

                     } else {
                         /* Notify in non transfer */
                     }
                     break;   
	/* Responses */
	...
	/* SIP responses */
		...
		case nua_r_message:
			...
			break;
                /* @pitzlabs response for REFER */
		case nua_r_refer:
	        JANUS_LOG(LOG_VERB, "[%s][%s]: %d %s\n", account->username, nua_event_name(event), status, phrase ? phrase : "??");
            /* handle REFER 202 */
            if(status == 202 && (session->transfer_status == janus_sip_transfer_initiated)) {
                session->transfer_status = janus_sip_transfer_transferee_accepted;  
            }
            break;
		...
		case nua_r_invite: {
			...
			if(status < 200) {
				...
				if(status == 180) {
					...
					int ret = gateway->push_event(account->handle, &janus_sip_plugin, account->transaction, ringing, NULL);
					JANUS_LOG(LOG_VERB, "  >> Pushing event to peer: %d (%s)\n", ret, janus_get_api_error(ret));
					json_decref(ringing);
					break;
				} else if(status == 183) {
				...
				if(account->authuser && strchr(account->authuser, ':')) {
					/* The authuser contains a colon: wrap it in quotes */
					g_snprintf(authuser, sizeof(authuser), "\"%s\"", account->authuser);
				} else {
					g_snprintf(authuser, sizeof(authuser), "%s", account->authuser);
				}
				if(account->secret && strchr(account->secret, ':')) {
					/* The secret contains a colon: wrap it in quotes */
					g_snprintf(secret, sizeof(secret), "\"%s\"", account->secret);
				} else {
					g_snprintf(secret, sizeof(secret), "%s", account->secret);
				}
				char auth[256];
				...
				g_snprintf(auth, sizeof(auth), "%s%s:%s:%s:%s%s",
					account->secret_type == janus_sip_secret_type_hashed ? "HA1+" : "",
					scheme,
					realm,
					authuser,
					account->secret_type == janus_sip_secret_type_hashed ? "HA1+" : "",
			...
			if(!in_progress && !session->media.autoack) {
				char *route = sip->sip_record_route ? url_as_string(account->stack->s_home, sip->sip_record_route->r_url) : NULL;
		...
		case nua_r_unregister: {
			...
			if(status == 200) {
				if(event == nua_r_register) {
					if(account->registration_status < janus_sip_registration_status_registered)
						account->registration_status = janus_sip_registration_status_registered;
				} else {
					account->registration_status = janus_sip_registration_status_unregistered;
				}
				...
				json_object_set_new(reging, "username", json_string(account->username));
				if(event == nua_r_register)
					json_object_set_new(reging, "register_sent", json_true());
				json_object_set_new(reg, "result", reging);
				JANUS_LOG(LOG_VERB, "Before Pushing event %s\n", event_name);
				int ret = gateway->push_event(account->handle, &janus_sip_plugin, account->transaction, reg, NULL);
				JANUS_LOG(LOG_VERB, "After Pushing event %s\n", event_name);
				JANUS_LOG(LOG_VERB, "  >> Pushing event to peer: %d (%s)\n", ret, janus_get_api_error(ret));
				json_decref(reg);
				/* Also notify event handlers */
				if(notify_events && gateway->events_is_enabled()) {
					...
					json_object_set_new(info, "identity", json_string(account->identity));
					if(account->proxy)
						json_object_set_new(info, "proxy", json_string(account->proxy));
					gateway->notify_event(&janus_sip_plugin, account->handle, info);
				}
			} else if(status == 401) {
				/* Get scheme/realm from 401 error */
				...
				account->registration_status = janus_sip_registration_status_failed;
				/* Tell the browser... */
				json_t *event = json_object();
				json_object_set_new(event, "sip", json_string("event"));
				json_t *result = json_object();
				...
				json_object_set_new(event, "result", result);
				int ret = gateway->push_event(account->handle, &janus_sip_plugin, account->transaction, event, NULL);
				JANUS_LOG(LOG_VERB, "  >> Pushing event to peer: %d (%s)\n", ret, janus_get_api_error(ret));
				json_decref(event);
				/* Also notify event handlers */
				if(notify_events && gateway->events_is_enabled()) {
...
static void janus_sip_connect_sockets(janus_sip_account *account, janus_sip_session *session, struct sockaddr_in *server_addr) {
	...
	if(session->media.remote_audio_rtp_port) {
		...
		if(connect(session->media.audio_rtp_fd, (struct sockaddr *)server_addr, sizeof(struct sockaddr)) == -1) {
			JANUS_LOG(LOG_ERR, "[SIP-%s] Couldn't connect audio RTP? (%s:%d)\n", account->username, session->media.remote_ip, session->media.remote_audio_rtp_port);
			JANUS_LOG(LOG_ERR, "[SIP-%s]   -- %d (%s)\n", account->username, errno, strerror(errno));
		}
	}
	if(session->media.remote_audio_rtcp_port) {
		...
		if(connect(session->media.audio_rtcp_fd, (struct sockaddr *)server_addr, sizeof(struct sockaddr)) == -1) {
			JANUS_LOG(LOG_ERR, "[SIP-%s] Couldn't connect audio RTCP? (%s:%d)\n", account->username, session->media.remote_ip, session->media.remote_audio_rtcp_port);
			JANUS_LOG(LOG_ERR, "[SIP-%s]   -- %d (%s)\n", account->username, errno, strerror(errno));
		}
	}
	if(session->media.remote_video_rtp_port) {
		...
		if(connect(session->media.video_rtp_fd, (struct sockaddr *)server_addr, sizeof(struct sockaddr)) == -1) {
			JANUS_LOG(LOG_ERR, "[SIP-%s] Couldn't connect video RTP? (%s:%d)\n", account->username, session->media.remote_ip, session->media.remote_video_rtp_port);
			JANUS_LOG(LOG_ERR, "[SIP-%s]   -- %d (%s)\n", account->username, errno, strerror(errno));
		}
	}
	if(session->media.remote_video_rtcp_port) {
		...
		if(connect(session->media.video_rtcp_fd, (struct sockaddr *)server_addr, sizeof(struct sockaddr)) == -1) {
			JANUS_LOG(LOG_ERR, "[SIP-%s] Couldn't connect video RTCP? (%s:%d)\n", account->username, session->media.remote_ip, session->media.remote_video_rtcp_port);
			JANUS_LOG(LOG_ERR, "[SIP-%s]   -- %d (%s)\n", account->username, errno, strerror(errno));
		}
	}
...
static void *janus_sip_relay_thread(void *data) {
    ...
    janus_sip_account *account = (janus_sip_account*)session->account_handle;
	if(!session || !account || !account->username || !session->callee) {
		g_thread_unref(g_thread_self());
		return NULL;
	}
	JANUS_LOG(LOG_VERB, "Starting relay thread (%s <--> %s)\n", account->username, session->callee);
	...
	if(have_server_ip)
		janus_sip_connect_sockets(account, session, &server_addr);
	...
	int astep = 0, vstep = 0;
	guint32 ats = 0, vts = 0;
	while(goon && session != NULL && !account->destroyed &&
			session->status > janus_sip_call_status_idle &&
		...
		if(session == NULL || account->destroyed ||
				session->status <= janus_sip_call_status_idle ||
				session->status >= janus_sip_call_status_closing)
			break;
		int i = 0;
		for(i=0; i<num; i++) {
			if(fds[i].revents & (POLLERR | POLLHUP)) {
				...
				if(error == 0) {
					...
					if(fds[i].fd == session->media.audio_rtcp_fd) {
						JANUS_LOG(LOG_WARN, "[SIP-%s] Got a '%s' on the audio RTCP socket, closing it\n",
							account->username, strerror(error));
						close(session->media.audio_rtcp_fd);
						session->media.audio_rtcp_fd = -1;
						continue;
				...
				JANUS_LOG(LOG_ERR, "[SIP-%s]   -- %d (%s)\n", account->username, error, strerror(error));
				...
				if(session->media.audio_rtp_fd != -1 && fds[i].fd == session->media.audio_rtp_fd) {
					...
					if(session->media.has_srtp_remote) {
						...
						if(res != srtp_err_status_ok && res != srtp_err_status_replay_fail && res != srtp_err_status_replay_old) {
							...
							JANUS_LOG(LOG_ERR, "[SIP-%s] Audio SRTP unprotect error: %s (len=%d-->%d, ts=%"SCNu32", seq=%"SCNu16")\n",
								account->username, janus_srtp_error_str(res), bytes, buflen, timestamp, seq);
							continue;
						}
						bytes = buflen;
					}
					...
					/* Relay to browser */
					gateway->relay_rtp(account->handle, 0, buffer, bytes);
					continue;
					...
					if(session->media.has_srtp_remote) {
						...
						if(res != srtp_err_status_ok && res != srtp_err_status_replay_fail && res != srtp_err_status_replay_old) {
							JANUS_LOG(LOG_ERR, "[SIP-%s] Audio SRTCP unprotect error: %s (len=%d-->%d)\n",
								account->username, janus_srtp_error_str(res), bytes, buflen);
							continue;
						}
						bytes = buflen;
					}
					/* Relay to browser */
					gateway->relay_rtcp(account->handle, 0, buffer, bytes);
					continue;
					...
					/* Relay to browser */
					gateway->relay_rtp(account->handle, 1, buffer, bytes);
					continue;
					...
					/* Relay to browser */
					gateway->relay_rtcp(account->handle, 1, buffer, bytes);
					continue;
				}
			}
		}
...
/* Sofia Event thread */
gpointer janus_sip_sofia_thread(gpointer user_data) {
	janus_sip_account *account = (janus_sip_account *)user_data;
	if(account == NULL || account->username == NULL) {
		g_thread_unref(g_thread_self());
		return NULL;
	}
	JANUS_LOG(LOG_VERB, "Joining sofia loop thread (%s)...\n", account->username);
	account->stack = g_malloc0(sizeof(ssip_t));
	account->stack->s_nua = NULL;
	account->stack->s_root = su_root_create(account->stack);
	su_home_init(account->stack->s_home);
	JANUS_LOG(LOG_VERB, "Setting up sofia stack (sip:%s@%s)\n", account->username, local_ip);
	char sip_url[128];
	char sips_url[128];
	char *ipv6;
	ipv6 = strstr(local_ip, ":");
    if(account->force_udp)
	...
	account->stack->s_nua = nua_create(account->stack->s_root,
				janus_sip_sofia_callback,
				account,
				SIPTAG_ALLOW_STR("INVITE, ACK, BYE, CANCEL, OPTIONS, UPDATE, MESSAGE, INFO"),
				NUTAG_M_USERNAME(account->username),
				NUTAG_URL(sip_url),
				TAG_IF(account->sips, NUTAG_SIPS_URL(sips_url)),
				SIPTAG_USER_AGENT_STR(account->user_agent ? account->user_agent : user_agent),
				...
				TAG_NULL());
	su_root_run(account->stack->s_root);
	/* When we get here, we're done */
	nua_destroy(account->stack->s_nua);
	su_root_destroy(account->stack->s_root);
	account->stack->s_root = NULL;
	su_home_deinit(account->stack->s_home);
	su_home_unref(account->stack->s_home);
	g_free(account->stack);
	account->stack = NULL;
	//~ stop = 1;
